...
class BaseSolarisConnector(initiator_connector.InitiatorConnector):
    platform = initiator.PLATFORM_ALL
    os_type = initiator.OS_TYPE_SOLARIS

    def __init__(self, root_helper, driver=None, execute=None,
                 *args, **kwargs):
        super(BaseSolarisConnector, self).__init__(root_helper, execute=execute,
                                                 *args, **kwargs)

        LOG.debug('BaseSolarisConnector initialized')
    ...
    def get_connector_properties(*args, **kwargs):
        ...
        props['multipath'] = (
            multipath and
            BaseSolarisConnector.check_multipath_support(enforce_multipath))
        return props
